"""Deploy cited-or-buried as a custom (FastAPI) Hopsworks app.

Thin client: reads geo_scored once at boot and calls the citedscorer deployment
for the coach. No heavy model in the pod, so it runs on stock python-app-pipeline
and uv-installs fastapi/uvicorn/python-multipart (pinned) in the entrypoint.
...
DEPS = "fastapi==0.115.6 uvicorn==0.32.1 python-multipart==0.0.12"
ENTRYPOINT = f'bash -lc "uv pip install --quiet --system {DEPS} && exec python {SERVER}"'
...
def _pods():
    ...
    return [l.split()[0] for l in out.splitlines() if l.split() and _is_app(l.split()[0])]


def _purge_k8s():
    out = subprocess.run(["kubectl", "get", "deployment"], capture_output=True, text=True).stdout
    for line in out.splitlines():
        if line.split() and _is_app(line.split()[0]):
            name = line.split()[0]
...
def _create(apps):
    return apps.create_app(
        ...
        description="cited-or-buried: does the AI answer quote your page, or just rank it? "
                    "A GEO ranker over search results, with a live page-scoring coach.")
